...
/**
 * Добавляем классы и обертку для сохранения
 */
const withSpoilerSave = createHigherOrderComponent((BlockListBlock) => {
    return (props) => {
        if (props.name !== 'core/image') {
        ...
        // Если спойлер не включен, возвращаем оригинальный блок
        if (!isSpoiler) {
            return <BlockListBlock {...props} />;
        }

        // Добавляем класс для спойлера
        const newProps = {
            ...props,
            className: `${props.className || ''} image-spoiler-wrapper`.trim(),
        };
...
addFilter(
    ...
    'image-spoiler/with-save',
    withSpoilerSave
);

/**
 * Модифицируем HTML на выходе
 */
function addSpoilerMarkup(element, blockType, attributes) {
    ...
    // Добавляем класс спойлера к img
    const modifiedImg = {
        ...imgElement,
        props: {
            ...imgElement.props,
            className: `${imgElement.props.className || ''} image-spoiler`.trim(),
        },
    };

    // Оборачиваем в div с текстом спойлера
    return (
        <div className="image-spoiler-wrapper">
            {modifiedImg}
            {spoilerText && (
                <div className="image-spoiler-text">
                    {spoilerText}
                </div>
            )}
        </div>
